...
public class ParkingLot {
	...
	public void park(Car c, int timestamp) {
		if(c == null){
			throw new IllegalArgumentException("We cannot park a car that doesn't exist!");
		}
		if(timestamp < 0){
			throw new IllegalArgumentException("A timestamp cannot be negative!");
		}
	...
	try {
		Spot removedCar = occupancy.get(i);
		occupancy.remove(i);
		return removedCar;
		}
	catch(ArrayIndexOutOfBoundsException e){
		throw new ArrayIndexOutOfBoundsException("This index is outside of the lot's boundaries!");
	}
	catch(NullPointerException e){
		throw new NullPointerException("Cannot remove a car that is not there!");
	}
	...
	public Spot getSpotAt(int i) {
		...
		Spot returnedCar = occupancy.get(i);
		return returnedCar;
	}
